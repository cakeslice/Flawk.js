...
export default function Layout(props: Props) {
	const { queryParams, setQueryParams, queryString } = useQueryParams<{
		page: number
		limit: number
		sort?: string
		order?: 'asc' | 'desc'
							...
							<Section
								description={
									<>
										...
										<sp />
										Use <code>path</code> prop to set the <m>parent path</m> of
										all pages.
										<br />
										Use <code>routes</code> prop to define each page path and
										their respective components, titles and icons.
										<br />
										Use <code>wrapperComponent</code> prop to set the{' '}
										<m>parent component</m> of all pages.
										<sp />
										On <m>desktop</m>, navigation controls are on a vertical bar
										on the left by default but can be changed to a horizontal
										top bar using the <code>horizontal</code> prop.
										<br />
										On <m>mobile</m>, navigation controls are on a vertical bar
										on the left and it uses the <code>{'<MobileDrawer/>'}</code>{' '}
...
>
	...
	<div style={gridExample}></div>
						...
						<Section
							description={
								<>
									To supply <m>data</m> to the table, use <code>data</code> prop
									which expects an array of objects.
									<br />
									To define the <m>table columns</m>, use <code>columns</code>{' '}
									prop. Each column has a <code>name</code> for the table header
									and a <code>selector</code> which maps a property of each data
									object to the column.
									<sp />
									Use <code>keySelector</code> prop to define a <m>main key</m>{' '}
									found in the data array. This key needs to be <m>unique</m> and
									usually is the id of each object or some other unique
									identifier.
									<sp />
									To <m>override or modify</m> the value displayed in a{' '}
									<m>specific column</m>, use the <code>cell</code> property when
									defining the column.
									<sp />
									<sp />
									The table component also has built-in <m>pagination</m>,{' '}
												...
												<div>
													<FButton
														onChange={(e) => {
															...
															if (e)
																selectedArray.push(d.id as string)
															else {
																_remove(
																	selectedArray,
																	(e) => e === d.id
						...
						<Section
							description={
								<>
									...
									displayed page
									<br />
									Use <code>totalPages</code> prop to set the <m>total</m> number
									of pages
									<sp />
...
`}
						...
						<Section
							description={
								<>
									The <code>{'<Modal/>'}</code> component is not <m>mounted</m>{' '}
									until {"it's"} displayed and where you place it in the component
									tree is <m>irrelevant</m>.
									<br />
									With the <code>content</code> prop you can set the content of
									the modal.
									<sp />
									Use <code>name</code> prop to define a <m>key</m> for the modal.
									This key will be expected to be in <code>this.state</code> and
									will be used to set if the modal is <m>closed</m> or <m>open</m>
									.
									<br />
									...
									<br />
									Use <code>onClose</code> prop to define a function to be called
...
`}
						...
						>
							<div className='wrapMargin flex flex-wrap justify-start'>
								<FButton onClick={() => exampleModalHook.setOpen(true)}>
									Default
								</FButton>
								<FButton
									appearance='delete'
						...
						<Section
							description={
								<>
									...
									<br />
									{"There's"} also different title colors depending on the type of
									the toast like <m>success</m>, <m>error</m> or <m>warning</m>.
									<sp />
...
`}
						...
						>
							<div
								className={
									desktop ? 'grid grid-cols-2' : 'wrapMarginBigVertical flex-col'
								}
								style={{ gap: 25 }}
							>
								<StatCard
						...
						<Section
							description={
								<>
									...
									<sp />
									Use <code>content</code> prop to display the content when the
									component is <m>expanded</m>.
								</>
							}
...
`}
									...
									>
										...
										>
											...
											<div style={{ alignSelf: 'flex-end' }}>
												<FButton onClick={() => setCollapse(false)}>
													{'Close'}
												</FButton>
											</div>
										</div>
						...
						<Section
							description={
								<>
									To make an element <m>sticky</m> across a <m>section</m> in your
									page, add a <m>unique</m> CSS class like{' '}
									<code>sticky_boundary</code> in the section <m>parent</m> and
									then use the <code>{'<Sticky/>'}</code> component as <m>one</m>{' '}
									of the children.
									<br />
									For the <code>boundaryElement</code> prop, supply it with the{' '}
									<m>same</m> unique CSS class you used in the parent.
									<sp />
			...
			{(desktop) => {
				return (
					...
					>
						...
						<hr style={{ margin: 0 }} />
						<div style={{ padding: p, height: 200, ...props.style }}>
							{props.children}
						</div>
